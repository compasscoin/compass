...
void QRCodeDialog::setModel(OptionsModel *model)
{
    ...
    // update the display unit, to not use the default ("CPC")
    updateDisplayUnit();
}
...
{
    QString ret = QString("compass:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non CPC unit input in lnReqAmount, we generate the URI with CPC as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::CPC, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
