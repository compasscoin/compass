...
bool CTxOut::IsDust() const
{
    ...
    // so dust is a txout less than 54 uCPC
...
void SetGenesisHash() {
        // Genesis Block:
        // CBlock(hash=000000000019d6, ver=1, hashPrevBlock=00000000000000, hashMerkleRoot=4a5e1e, nTime=1397053046, nBits=1d00ffff, nNonce=2083236893, vtx=1)
        ...
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 0xc5dcda7e << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
...
unsigned char pchMessageStart[4] = { 0xc5, 0xdc, 0xda, 0x7e };
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("CompassMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("CompassMiner : ProcessBlock, block not accepted");
    }
...
{
    printf("CompassMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("compass-miner");
    ...
    try { loop {
        ...
        printf("Running CompassMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("CompassMiner terminated\n");
        throw;
    }
}
